function update_U(datC, datT, cur, hyper)
    ...
    for h in 1:(NT+nT)
        ...
        for l in 1:(_g+gstar) 
            ...
            if h <= NT
                i,j = h2ij(h, NvecT)
                logLprob[l] += logpdf(LogLogistic(_lambda[l] / xiT[i], _eta[l]), gapT[i][j])
            else
                i = h - NT
                if NvecT[i] == 0 
                    logLprob[l] += logccdf(LogLogistic(_lambda[l] / xiT[i], _eta[l]), survivalT[i])
                else
                    logLprob[l] += logccdf(LogLogistic(_lambda[l] / xiT[i], _eta[l]), survivalT[i] - sum(gapT[i]))
                end 
            end
        end
