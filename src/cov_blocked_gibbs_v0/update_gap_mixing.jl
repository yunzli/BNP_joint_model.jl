...
function update_gap_pg_lantent(datC, datT, cur) 
    ...
    for h in 1:(NC+nC)
        ...
        if h <= NC
            i, j = h2ij(h, NvecC)
            b = eta[tUh] * (log(gapC[i][j]) - gamma[tUh,:]'*zC[i,:] + log(xiC[i]) - log(lambda[tUh]))
            varsigmaC[h] = rand(PolyaGamma(a, b), 1)[1]
        else
            i = h - NC 
            if NvecC[i] == 0
                b = eta[tUh] * (log(survivalC[i]) - gamma[tUh,:]'*zC[i,:] + log(xiC[i]) - log(lambda[tUh]))
            else
                b = eta[tUh] * (log(survivalC[i]-sum(gapC[i])) - gamma[tUh,:]'*zC[i,:] + log(xiC[i]) - log(lambda[tUh]))
            end
    ...
    for h in 1:(NT+nT)
        ...
        if h <= NT
            i, j = h2ij(h, NvecT)
            b = eta[tUh] * (log(gapT[i][j]) - gamma[tUh,:]'*zT[i,:] + log(xiT[i]) - log(lambda[tUh]))
            varsigmaT[h] = rand(PolyaGamma(a, b), 1)[1]
        else
            i = h - NT 
            if NvecT[i] == 0
                b = eta[tUh] * (log(survivalT[i]) - gamma[tUh,:]'*zT[i,:] + log(xiT[i]) - log(lambda[tUh]))
            else
                b = eta[tUh] * (log(survivalT[i]-sum(gapT[i])) - gamma[tUh,:]'*zT[i,:] + log(xiT[i]) - log(lambda[tUh]))
            end
...
function update_gamma(datC, datT, cur, hyper)
    ...
    for l in 1:BH 
        ...
        if (length(indC) == 0) & (length(indT) == 0)
            ...
            mu_gamma_new = Sigma_gamma_new * (Sigma_gamma * mu_gamma + eta[l]^2 * tmp1 + 0.5 * eta[l] * tmp2)
