function update_L(datC, datT, cur, hyper)
    ...
    for i in 1:nT
        ...
        for l in 1:(_k+kstar) 
            ...
            if nuT[i] == 1
                logLprob[l] += logpdf(LogLogistic(_theta[l] / epsilonT[i], _phi[l]), survivalT[i])
            else
                logLprob[l] += logccdf(LogLogistic(_theta[l] / epsilonT[i], _phi[l]), survivalT[i])
            end
        end
    ...
    return Dict("L" => L, "theta" => theta, "phi" => phi, "k" => k, "nl" => nl)
end