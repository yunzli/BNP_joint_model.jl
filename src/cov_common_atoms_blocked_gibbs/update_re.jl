function update_random_effects(datC, datT, cur, hyper, Sig)
    ...
    for i in 1:nT
        ...
        Li = LT[i]
        if nuT[i] == 1
            l_cur += logpdf(LogLogistic(theta[Li]/epsilon_i_cur, phi[Li]), survivalT[i])
            l_pro += logpdf(LogLogistic(theta[Li]/epsilon_i_pro, phi[Li]), survivalT[i])
        else 
            l_cur += logccdf(LogLogistic(theta[Li]/epsilon_i_cur, phi[Li]), survivalT[i])
            l_pro += logccdf(LogLogistic(theta[Li]/epsilon_i_pro, phi[Li]), survivalT[i])
        end
        ...
        if NvecT[i] > 0
            for j in 1:NvecT[i]
                h = ij2h(i,j,NvecT)
                tUh = tUT[h]
                l_cur += logpdf(LogLogistic(lambda[tUh]/xi_i_cur, eta[tUh]), gapT[i][j])
                l_pro += logpdf(LogLogistic(lambda[tUh]/xi_i_pro, eta[tUh]), gapT[i][j])
            end
        end
        ...
        tUh = tUT[h]
        if NvecT[i] == 0 # N
            l_cur += logccdf(LogLogistic(lambda[tUh]/xi_i_cur, eta[tUh]), survivalT[i])
            l_pro += logccdf(LogLogistic(lambda[tUh]/xi_i_pro, eta[tUh]), survivalT[i])
        else
            l_cur += logccdf(LogLogistic(lambda[tUh]/xi_i_cur, eta[tUh]), survivalT[i]-sum(gapT[i]))
            l_pro += logccdf(LogLogistic(lambda[tUh]/xi_i_pro, eta[tUh]), survivalT[i]-sum(gapT[i]))
        end 
