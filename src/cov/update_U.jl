function update_U(dat, cur, hyper)
    ...
    for h in 1:(N+n)
        ...
        for l in (_g+1):(_g+gstar)
            ...
            _gamma[l,:] = rand(MvNormal(mu_gamma, Sigma_gamma), 1)
        ...
        for l in 1:(_g+gstar) 
            ...
            if h <= N
                i,j = h2ij(h, Nvec)
                logLprob[l] += logpdf(LogLogistic(_lambda[l] * exp(_gamma[l,:]'*z[i,:]) / xi[i], _eta[l]), gap[i][j])
            else
                i = h - N
                if Nvec[i] == 0 
                    logLprob[l] += logccdf(LogLogistic(_lambda[l] * exp(_gamma[l,:]'*z[i,:]) / xi[i], _eta[l]), survival[i])
                else
                    logLprob[l] += logccdf(LogLogistic(_lambda[l] * exp(_gamma[l,:]'*z[i,:]) / xi[i], _eta[l]), survival[i] - sum(gap[i]))
                end 
            end
        end
